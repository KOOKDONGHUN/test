...
class Post(models.Model): #models의 model을 상속 받겠다
    #ㅍㅣㄹ드명---
    ...
    description = models.CharField('DESCRIPTION',max_length=100,blank=True,help_text='간단한 설명문')
    ...
    class Meta:#테이블 자체의 정보
        ...
        db_table = 'my_post' #데이터 베이스에 저장되는 테이블명     # 테이블 명이 된다?
    ...
    def get_absolute_url(self):
        return reverse('notice:post_detail', args=(self.slug,)) # reverse 블로그에 대한 url을 출력?
                                                            # slug약어 사용가능?
    # 현재 포스트를 기준으로 이전 포스트 반환??
